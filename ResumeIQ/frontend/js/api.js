...
/* ── Determine API base relative to current page ─────────── */
...
const DashboardAPI = { get: () => apiFetch("dashboard.php") };

const ProfileAPI = {
  get: () => apiFetch("profile.php"),
  update: (data) =>
    apiFetch("profile.php?action=update", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  changePassword: (c, n, cf) =>
    apiFetch("profile.php?action=password", {
      method: "POST",
      body: JSON.stringify({
        current_password: c,
        new_password: n,
  ...
  saveNotifPrefs: (prefs) =>
    apiFetch("profile.php?action=notif", {
      method: "POST",
  ...
  uploadResume: (file) => {
    ...
    return apiUpload("profile.php?action=resume", fd);
  },
};

const JobsAPI = {
  list: (p = {}) => apiFetch(`jobs.php?action=list&${new URLSearchParams(p)}`),
  apply: (id) =>
    apiFetch("jobs.php?action=apply", {
      method: "POST",
      body: JSON.stringify({ job_id: id }),
    }),
  seed: () => apiFetch("jobs.php?action=seed"),
  create: (data) =>
    apiFetch("jobs.php?action=create", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  myJobs: () => apiFetch("jobs.php?action=myjobs"),
  close: (id) =>
    apiFetch("jobs.php?action=close", {
      method: "POST",
      body: JSON.stringify({ job_id: id }),
    }),
  del: (id) =>
    apiFetch("jobs.php?action=delete", {
      method: "POST",
      body: JSON.stringify({ job_id: id }),
    }),
};
...
const ApplicationsAPI = {
  list: (p = {}) =>
    apiFetch(`applications.php?action=list&${new URLSearchParams(p)}`),
  withdraw: (app_id) =>
    apiFetch("applications.php?action=withdraw", {
      method: "POST",
      body: JSON.stringify({ app_id }),
    }),
  stats: () => apiFetch("applications.php?action=stats"),
  forJob: (job_id, p = {}) =>
    apiFetch(
      `applications.php?action=for_job&job_id=${job_id}&${new URLSearchParams(p)}`,
    ),
  updateStatus: (app_id, status, notes = "") =>
    apiFetch("applications.php?action=update_status", {
      method: "POST",
      body: JSON.stringify({ app_id, status, notes }),
    }),
  hrStats: () => apiFetch("applications.php?action=hr_stats"),
  allForHR: (p = {}) =>
    apiFetch(`applications.php?action=all_hr&${new URLSearchParams(p)}`),
};

const HRAPI = {
  dashboard: () => apiFetch("hr_dashboard.php"),
  candidates: (p = {}) =>
    apiFetch(`hr_dashboard.php?action=candidates&${new URLSearchParams(p)}`),
};
