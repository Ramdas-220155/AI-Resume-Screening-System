  ...
  function renderTable(apps) {
    ...
    tbody.innerHTML = apps
      .map(
        (a) => `
      <tr data-id="${a.id}">
        <td><div><div style="font-family:var(--ff-head);font-size:14px;font-weight:700;color:var(--clr-head);">${escHtml(a.title)}</div><div style="font-size:12px;color:var(--clr-muted);">${escHtml(a.type || "")}</div></div></div></td>
