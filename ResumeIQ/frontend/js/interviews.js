...
function renderList(list) {
  ...
  container.innerHTML = list.map(iv => {
    ...
    const canAccept     = iv.status === 'scheduled';
    const canReschedule = (iv.status === 'scheduled' || iv.status === 'accepted') && iv.reschedule_status !== 'pending';
    ...
    return `<div class="iv-card">
      ...
      <div class="iv-card-actions">
        ${canAccept ? `<button class="btn btn-primary btn-sm" onclick="acceptInterview('${iv.id}')"><i class="fa-solid fa-check"></i> Accept Invitation</button>` : ''}
        ${isAccepted ? `<button class="btn btn-sm" style="background:rgba(16,185,129,0.1);color:#10b981;border:1px solid rgba(16,185,129,0.3);cursor:default;" disabled><i class="fa-solid fa-circle-check"></i> Accepted</button>` : ''}
        ${canReschedule ? `<button class="btn btn-outline btn-sm" onclick="openReschedule('${iv.id}')"><i class="fa-solid fa-calendar-xmark"></i> Request Reschedule</button>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
async function acceptInterview(id) {
