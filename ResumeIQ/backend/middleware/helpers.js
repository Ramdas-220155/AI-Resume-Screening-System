...
async function requireAuth(req, res) {
  ...
  if (!id) { fail(res, 'Unauthorized', 401); return null; }
  try {
    const users = await getCol('users');
    const u = await users.findOne({ _id: toObjId(id) });
    if (!u) { fail(res, 'Unauthorized — user not found', 401); return null; }
    return u;
  } catch (e) {
...
async function requireHR(req, res) {
  ...
  if (!u) return null;
  if ((u.role || 'user') !== 'hr') { fail(res, 'Forbidden — HR access required', 403); return null; }
  return u;
}
